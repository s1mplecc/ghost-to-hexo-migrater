...
部署到 GitHub Pages 上会提供 `https://${github-username}.github.io` 默认的域名以供访问。除此之外，GitHub Pages 也提供绑定个性域名的功能。
...
第二步，在本地 Hexo 的 `source` 文件夹下添加**名为 CNAME 的文件**，内容为个性域名地址，比如我的 `s1mple.xyz`。并执行 `hexo g -d` 部署到服务器。
